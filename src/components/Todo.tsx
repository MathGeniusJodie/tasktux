...
export const TodoListItem: React.FC<TodoProps> = ({
  todo,
  toggleTodo,
  deleteTodo,
  updateTodo,
  ...
  <div
    className={cn(
      "flex items-center border rounded h-18 gap-4 px-4",
      className
    )}
    style={{ flexShrink: "0" }}
  >
    ...
    <Label
      ...
      className={
        cn(todo.completed ? "line-through flex-grow h-16" : "flex-grow h-20", labelClassName)
      }
    >
      {todo.text}
    </Label>
    ...
    }
    {
      ...
      "endDate" in todo &&
        ...
        todo.endDate <=
          new Date(
            Date.now() +
              ("storyPoints" in todo && todo.storyPoints
                ? todo.storyPoints / 6 + 1
                : 1) *
                24 *
                60 *
                60 *
    ...
    <Popover>
      ...
      <PopoverContent className="w-80">
        ...
        >
          {possibleStoryPoints.map((point) => (
            <ToggleGroupItem
              ...
              value={point}
              aria-label={
                point === "🤷" ? "No story points" : `Story points ${point}`
              }
            >
              {point}
            </ToggleGroupItem>
        ...
        <ToggleGroup
          ...
          defaultValue={
            "priority" in todo && todo.priority !== undefined
